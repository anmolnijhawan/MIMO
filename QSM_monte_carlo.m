...
EbNo = 0:2:26;  %Range of SNR for simulation
...
for itr = 1:1:length(EbNo)  % Iterate for each value of SNR
   for sym = 1:N            % Iterate for each symbol
       ...
       x_inphase  = zeros(Nt,1); 
       x_quadrature=zeros(Nt,1);% Initialize all antennas
       x_inphase(r_ant_index) = real(x_i);                            % Symbol transmiited from respective antenna
       x_quadrature(i_ant_index) = imag(x_i);
       x=x_inphase+x_quadrature*i;
       y = H*x;                                        
       ...
       for m=0:1:M-1
           for l_r=1:1:Nt
               for l_i=1:1:Nt
                   ...
                   x_r = real(x_);
                   x_i = imag(x_);
                   g = H(:,l_r)*x_r + H(:,l_i)*x_i*i; 
                   val = norm(g)*norm(g) - 2*real(r'*g);
                   if val<min
                       min = val;
...
error_rate_QSM = error/N;
semilogy(EbNo,error_rate_QSM,'r');
grid on;
hold on;
